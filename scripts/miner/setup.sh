...
fix_scalecodec_conflict() {
  info_msg "Resolving scalecodec vs cyscale conflict for bittensor SDK..."
  pip uninstall scalecodec -y >/dev/null 2>&1 || true
  pip install cyscale --force-reinstall -q || handle_error "Failed to reinstall cyscale"
  success_msg "cyscale active (scalecodec removed from miner venv)."
}

verify_installation() {
  info_msg "Verifying participant environment setup..."
  python -c "import bittensor; print(f'✓ Bittensor: {bittensor.__version__}')" || handle_error "Bittensor import failed after cyscale fix"
